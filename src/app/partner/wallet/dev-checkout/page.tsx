  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        router.push(data.purpose === "CASE_PAYMENT" ? "/partner/dashboard" : "/partner/wallet");
        router.refresh();
      } catch {
        setStatus("error");
  ...
  return (
    <div className="w-full max-w-sm rounded-sm border border-line bg-white p-8 text-center">
      ...
      <p className="mt-6 text-xs text-ink/40">
        In production this page is replaced entirely by a real redirect to PayU&apos;s hosted checkout — see
        src/lib/payment.ts.
      </p>
    </div>
  );
