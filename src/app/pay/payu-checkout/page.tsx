...
/**
 * Shared PayU handoff for partner + consumer orders.
 * Settlement goes to the Visamile merchant account (PAYMENT_GATEWAY_KEY).
 */
export default async function PayUCheckoutPage({
  ...
  if (!session || (session.role !== "PARTNER" && session.role !== "CONSUMER")) redirect("/login");
  ...
  if (session.role === "PARTNER" && order.partnerId !== session.partnerId) notFound();
  if (session.role === "CONSUMER" && order.consumerUserId !== session.sub) notFound();

  const consumer = order.consumerUserId
    ? await prisma.user.findUnique({
        where: { id: order.consumerUserId },
  ...
  const productinfo = order.purpose === "CASE_PAYMENT" ? "Visa case payment — Visamile" : "Wallet top-up";
  const firstname = order.partner?.companyName || consumer?.name || session.name || "Customer";
  const email = order.partner?.contactEmail || consumer?.email || session.email;
  const phone = order.partner?.contactPhone || "9999999999";
  const hashString = [
    key,
    orderId,
  ...
  return (
    <main className="mx-auto max-w-lg px-6 py-16">
      <h1 className="text-xl font-medium">Redirecting to payment portal…</h1>
      <p className="mt-2 text-sm text-ink/60">
        Amount payable to Visamile: ₹{fee.totalPayable.toFixed(2)} (incl. gateway fee + GST)
      </p>
